      ...
      <main className="flex justify-center p-5 rounded-2xl w-[600px] h-[450px] shadow-md shadow-white drop-shadow-xl backdrop-blur-2xl shadow-inner bg-white/35 border-[2px] ">
        {submit ? <span className="flex flex-col justify-between w-full">
          ...
          <section className="py-5 flex flex-col justify-between">
            <p className="text-center py-6 h-[100px] text-lg text-gray-600 font-semibold">{questions[showQuestion].question}</p>
        ...
        <span className="flex flex-col items-center justify-center w-full space-y-10">
          ...
          <span className="w-20 h-20 flex justify-center items-center rounded-[50%] shadow-md"><span>{correctAnswer}</span>/ <span>{questions.length}</span></span>
          {}
          <h2 className="text-lg font-medium">{failed ? "Try reduce your Sleep!" : "Congratulations" }</h2>
          <button onClick={handleSubmit} className={button}>{failed ? "Redeem Yourself!" : "One More"}</button>
        </span> }
